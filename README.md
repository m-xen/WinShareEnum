...
The ‘Interesting Files’ feature contains a (configurable) list of default words, wildcards and regular expressions to configure a search of all discovered shared folders,
to focus analysis into any potentially dangerous storage of information (such as user credentials) based on the filename(s).

The ‘File Contents’ feature contains a (configurable) list of default words, and can also contain wildcards & regular expressions,
to search the contents of files within discovered shared folders, to focus analysis into any potentially dangerous storage of information (such as user credentials)
